  ...
  return (
    <>
      ...
      <h3 className="text-center">
        page <span className="text-info d-inline">{allMovies.page}</span> of <span className="text-info d-inline">500</span>
      </h3>
      ...
      <div className="container my-5 justify-content-evenly">
        {allMovies.results ? (
          <div>
            <div className="row">
              ...
              {/*  🔥 فلتر منع الأفلام الإباحية adult === false  */}
              {allMovies.results
                ...
                .map((movie) => (
                  <div key={movie.id} className="col-lg-4 text-center my-5">
                    <Link to={`/movieDetails/${movie.id}`}>
                      <img
                        onClick={() => dispatch(getOneMovie(movie.id))}
                        loading="lazy"
                        className="TopRatedMoviesImages w-50 my-1"
                    ...
                    <ShowMore
                      className={isDark ? "bg-black text-secondary" : "bg-light text-dark"}
                      maxHeight={62}
                      button={<button type="button" className="btn btn-dark">movie details</button>}
                    >
                      <p>{movie.overview}</p>
                    </ShowMore>
              ...
              <nav aria-label="...">
                <ul className="pagination">
                  <li className="page-item">
                    <button onClick={() => dispatch(getMovies(allMovies.page - 1))} className="page-link">
                    ...
                    return (
                      <li className="page-item" key={num}>
                        <button
                          ...
                          className={num === 0 ? 'active page-link' : 'page-link'}
                        >
                          {page}
                        </button>
                      </li>
        ...
        <div className="text-center my-5">
          <h6 className="text-center my-2">
            page <span className="text-info d-inline my-2">{allMovies.page}</span> of <span className="text-info d-inline">500</span>
          </h6>
        </div>
      </div>
    </>
